...
void final(int *T_, int size)
{
   for (int i = 0; i < (size - 1); ++i)
      if ((i + 1) <= size && T_[i] > T_[i + 1]) swapRefs(T_, i, i + 1);
}

void printT(int *T_, int size)
{
   for (int i = 0; i < size; ++i) {
      if (i == 0) printf("[%d, ", T_[i]);
...
{
   if (left + CUTOFF > right) return; // Tableau de plus de 2 elements uniquement a trier. On devrait faire un autre type d'insertion dans ce cas, mais nous simplifions.
   ...
   for (;;) {
      ...
      while (T_[--k] > pivot)
         ;
      if (i < k) swapRefs(T_, i, k);   // On permute T[i] avec T[k]
      else break;                      // Sortir car partitionnement termine
   }
...
int medianOfThree(int *T_, int left, int right)
{
   ...
   if (T_[center] < T_[left])    swapRefs(T_, left, center);
   if (T_[right] < T_[left])     swapRefs(T_, left, right);
   if (T_[right] < T_[center])   swapRefs(T_, center, right);
